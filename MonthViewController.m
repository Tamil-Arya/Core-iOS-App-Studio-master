...
{
    NSLog(@"Date Selected");
    if ([self.delegate respondsToSelector:@selector(monthViewController:didSelectdayAtDate:)]) {
        [self.delegate monthViewController:self didSelectdayAtDate:date];
    }
}
...
- (NSAttributedString*)monthPlannerView:(MGCMonthPlannerView *)view attributedStringForDayHeaderAtDate:(NSDate *)date
{
    ...
    NSMutableArray * array = [[NSUserDefaults standardUserDefaults] objectForKey:@"arrayWithDatesFromWebService"];
    //    NSLog(@"arrayWithDatesFromWebService %@ %@",array,date);
