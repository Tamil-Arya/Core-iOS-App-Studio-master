...
@interface PortfolioWebViewController ()
{
    APIWithProtocol * api;
}
@end

@implementation PortfolioWebViewController
static NSInteger WEBSERVICE_TO_GET_Portfolio_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
    [self Navigation_bar];
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"portfolio",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Portfolio_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
  ...
  urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=portfolio&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
    } else {
        urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=portfolio",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
    }
    [self.portfolioWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
//    UIImage *butImage = [[UIImage imageNamed:@"back-2"] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Portfolio" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.portfolioWebView canGoBack]) {
        [self.portfolioWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_Portfolio_URL_TAG) {
        [self mStopIndicater];
