//
//  Single_absent_note_Notification.m
...
@interface Single_absent_note_Notification ()

@end

@implementation Single_absent_note_Notification
...
//        NSString *date_correct_format = [dateFormatter stringFromDate:date]; //here convert date in NSString
//        
        NSString *responseString = [API makeCallPostData_ALL:[NSString stringWithFormat:@"securityKey=%@&authentication_token=%@&language=%@&student_id=%@&sickdate=%@",@"H67jdS7wwfh",Token_value,[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],[[array_all_detail valueForKey:@"object"]valueForKey:@"id"],[[array_all_detail valueForKey:@"object"]valueForKey:@"date"]]:[NSString stringWithFormat:@"%@retrivals/get_absence_note",[Utilities API_link_url_subDomain]]];
        ...
        if ([[dict valueForKey:@"status"] isEqualToString:@"true"]) {
            ...
            writtenbyvaluelabel = [[UILabel alloc] initWithFrame:CGRectMake(writtenbylabel.frame.origin.x+writtenbylabel.frame.size.width+5, absentnotedesclabel.frame.origin.y+absentnotedesclabel.frame.size.height+5  ,[[[dict valueForKey:@"data"]valueForKey:@"written_by"] sizeWithFont:[Font_Face_Controller opensansLight:16]].width, 20)];
