//
//  EduStepPlannerWebViewController.m
...
@interface EduStepPlannerWebViewController ()
{
    APIWithProtocol * api;
}
@end

@implementation EduStepPlannerWebViewController
static NSInteger WEBSERVICE_TO_GET_Edu_Step_Planner_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"edu_step_planner",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Edu_Step_Planner_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
    ...
    urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=edu_step_planner&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
    } else {
        urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=edu_step_planner",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
    }
    [self.eduStepPlannerWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
-(void)Navigation_bar
{
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Edu Step Planner" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.eduStepPlannerWebView canGoBack]) {
        [self.eduStepPlannerWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_Edu_Step_Planner_URL_TAG) {
        [self mStopIndicater];
