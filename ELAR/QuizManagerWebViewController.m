//
//  QuizManagerWebViewController.m
...
@interface QuizManagerWebViewController ()
{
    APIWithProtocol * api;
}
...
@implementation QuizManagerWebViewController
static NSInteger WEBSERVICE_TO_GET_QuizManager_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"quiz_manager",@"componenet_name",@"H67jdS7wwfh",@"securityKey",loginUserId,@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_QuizManager_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
   ...
   urlString  = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=quiz_manager&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
    } else {
         urlString  = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=quiz_manager",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
    }
    [self.quizManagerWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
//    UIImage *butImage = [[UIImage imageNamed:@"back-2"] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Quiz Manager" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.quizManagerWebView canGoBack]) {
        [self.quizManagerWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_QuizManager_URL_TAG) {
        [self mStopIndicater];
