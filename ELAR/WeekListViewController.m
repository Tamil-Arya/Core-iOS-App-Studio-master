...
static NSInteger WEBSERVICE_TO_GET_Week_List_URL_TAG = 201;
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"week_list",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Week_List_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
  ...
  urlString   = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=week_list&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
    NSLog(@"url=====>%@",urlString);
     } else {
          urlString   = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=week_list",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
     }
    [self.weekListWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
-(void)Navigation_bar
{
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Week view" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.weekListWebView canGoBack]) {
        [self.weekListWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_Week_List_URL_TAG) {
        [self mStopIndicater];
