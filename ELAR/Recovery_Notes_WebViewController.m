//
//  Recovery_Notes_WebViewController.m
...
@interface Recovery_Notes_WebViewController ()
{
    APIWithProtocol * api;
}
@end

@implementation Recovery_Notes_WebViewController
static NSInteger WEBSERVICE_TO_GET_Recovery_Notes_URL_TAG = 201;
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"recovery_notice",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Recovery_Notes_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
    ...
    urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=recovery_notice&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
    NSLog(@"url=====>%@",urlString);
     } else {
         urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=recovery_notice",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
     }
    [self.recoveryNotesWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
//    UIImage *butImage = [[UIImage imageNamed:@"back-2"] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Recovery Notice" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.recoveryNotesWebView canGoBack]) {
        [self.recoveryNotesWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_Recovery_Notes_URL_TAG) {
        [self mStopIndicater];
