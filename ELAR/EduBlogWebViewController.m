...
@interface EduBlogWebViewController ()
{
    APIWithProtocol * api;
}
@end

@implementation EduBlogWebViewController
static NSInteger WEBSERVICE_TO_GET_Edu_Blog_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
    [self Navigation_bar];
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"edublog",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Edu_Blog_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
    ...
    NSString *urlString;
    if ([[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"parent"] || [[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"förälder"]) {
   urlString  = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=edublog&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
    
    NSLog(@"webURL====>%@",urlString);
    } else {
        urlString  = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=edublog",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
        
    }
    [self.eduBlogWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
- (IBAction)backButtonClicked:(id)sender {
    if ([self.eduBlogWebView canGoBack]) {
        [self.eduBlogWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_Edu_Blog_URL_TAG) {
        [self mStopIndicater];
