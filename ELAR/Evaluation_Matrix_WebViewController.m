//
//  Evaluation_Matrix_WebViewController.m
...
@interface Evaluation_Matrix_WebViewController ()
{
    APIWithProtocol * api;
}
@end

@implementation Evaluation_Matrix_WebViewController
static NSInteger WEBSERVICE_TO_GET_Evaluation_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
    [self Navigation_bar];
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"evaluation_tables",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Evaluation_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
    ...
    NSString *urlString;
     if ([[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"parent"] || [[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"förälder"]) {
    urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=evaluation_tables&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
     } else {
           urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=evaluation_tables",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
     }
    [self.evaluationMatrixWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
//    UIImage *butImage = [[UIImage imageNamed:@"back-2"] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Evaluation Matrix" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.evaluationMatrixWebView canGoBack]) {
        [self.evaluationMatrixWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_Evaluation_URL_TAG) {
        [self mStopIndicater];
