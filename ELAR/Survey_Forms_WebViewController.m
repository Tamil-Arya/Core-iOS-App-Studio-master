//
//  Survey_Forms_WebViewController.m
...
@interface Survey_Forms_WebViewController ()
{
    APIWithProtocol * api;
}
@end

@implementation Survey_Forms_WebViewController
static NSInteger WEBSERVICE_TO_GET_Survey_Forms_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
    [self Navigation_bar];
...
-(void) webserviceToGetWebViewURL  {
    ...
    if ([[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"parent"] || [[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"förälder"]) {
   postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"survey_forms_students",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    } else {
       postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"survey_manager",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    }
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Survey_Forms_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
    ...
    NSString *urlString;
    if ([[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"parent"] || [[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"förälder"]) {
   urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=survey_forms_students&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
    [self.surveyformsWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
    
    } else {
        urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=survey_manager",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
        [self.surveyformsWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
    }
...
//    UIImage *butImage = [[UIImage imageNamed:@"back-2"] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Survey Forms" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.surveyformsWebView canGoBack]) {
        [self.surveyformsWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_Survey_Forms_URL_TAG) {
        [self mStopIndicater];
