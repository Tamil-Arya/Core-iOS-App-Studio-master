...
@interface NewsBlogWebViewController ()
{
    APIWithProtocol * api;
}
@end

@implementation NewsBlogWebViewController
static NSInteger WEBSERVICE_TO_GET_News_Blog_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
    [self Navigation_bar];
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"news",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_News_Blog_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
   ...
   urlString  = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=news&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
    } else {
        urlString  = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=news",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
    }
    [self.newsBlogWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
- (IBAction)backButtonClicked:(id)sender {
    if ([self.newsBlogWebView canGoBack]) {
        [self.newsBlogWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_News_Blog_URL_TAG) {
        [self mStopIndicater];
