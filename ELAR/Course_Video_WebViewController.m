//
//  Course_Video_WebViewController.m
...
@interface Course_Video_WebViewController ()
{
    APIWithProtocol * api;
}
@end

@implementation Course_Video_WebViewController
static NSInteger WEBSERVICE_TO_GET_Course_Video_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
    [self Navigation_bar];
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"course_videos",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Course_Video_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
    ...
    NSString *urlString;
     if ([[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"parent"] || [[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"förälder"]) {
   urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=course_videos&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
     } else {
          urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=course_videos",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
     }
    [self.courseVideoWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
//    user_pic.frame=CGRectMake(50, 0, 30, 30);
 ...
 //   UIImage *butImage = [[UIImage imageNamed:@"back-2"] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Course Videos" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.courseVideoWebView canGoBack]) {
        [self.courseVideoWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_Course_Video_URL_TAG) {
        [self mStopIndicater];
