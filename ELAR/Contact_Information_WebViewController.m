//
//  Contact_Information_WebViewController.m
...
@interface Contact_Information_WebViewController ()
{
    APIWithProtocol * api;
}
...
@implementation Contact_Information_WebViewController
static NSInteger WEBSERVICE_TO_GET_Contact_Information_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
    [self Navigation_bar];
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"contact_information",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Contact_Information_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
    ...
    urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=contact_information&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
     } else {
           urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=contact_information",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
     }
    [self.contactInformationWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
//    user_pic.frame=CGRectMake(50, 0, 30, 30);
 ...
 //   UIImage *butImage = [[UIImage imageNamed:@"back-2"] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Contact Information" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.contactInformationWebView canGoBack]) {
        [self.contactInformationWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_Contact_Information_URL_TAG) {
        [self mStopIndicater];
