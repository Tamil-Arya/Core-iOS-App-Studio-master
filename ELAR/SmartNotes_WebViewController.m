//
//  SmartNotes_WebViewController.m
...
@interface SmartNotes_WebViewController ()<APIProtocol>
{
    APIWithProtocol * api;
}
@end

@implementation SmartNotes_WebViewController
static int WEBSERVICE_TO_GET_Smart_Notes_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void) webserviceToGetWebViewURL  {
    ...
    if ([[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"parent"] || [[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"förälder"]) {
          postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"smart_notes_parent",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    } else {
        postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"smart_notes_wall",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    }
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Smart_Notes_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
    ...
    NSString *urlString;
     ...
     if ([[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"parent"] || [[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"förälder"]) {
         urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=smart_notes_parent&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
         NSLog(@"url=====>%@",urlString);
     } else {
         urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=smart_notes_wall",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
         NSLog(@"url=====>%@",urlString);
     }
    
    [self.smartNotesWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
-(void)Navigation_bar
{
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Latest" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.smartNotesWebView canGoBack]) {
        [self.smartNotesWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_Smart_Notes_URL_TAG) {
        [self mStopIndicater];
