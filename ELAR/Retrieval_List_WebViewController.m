//
//  Retrieval_List_WebViewController.m
...
@interface Retrieval_List_WebViewController ()
{
    APIWithProtocol * api;
}
@end

@implementation Retrieval_List_WebViewController
static NSInteger WEBSERVICE_TO_GET_Retrieval_List_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
    [self Navigation_bar];
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"attendance",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Retrieval_List_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
   ...
   urlString  = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=attendance&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
     } else {
         urlString  = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=attendance",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
     }
    [self.retrievalListWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
//    UIImage *butImage = [[UIImage imageNamed:@"back-2"] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Retrieval List" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.retrievalListWebView canGoBack]) {
        [self.retrievalListWebView goBack];
    }
    else
    {
...
{
    [self mStartIndicater];
    //self.retrievalListWebView.scalesPageToFit = true;
}
...
{
    if (msgType == WEBSERVICE_TO_GET_Retrieval_List_URL_TAG) {
        [self mStopIndicater];
