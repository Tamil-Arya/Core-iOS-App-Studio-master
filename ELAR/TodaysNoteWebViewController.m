...
@interface TodaysNoteWebViewController ()
{
    APIWithProtocol * api;
}
@end

@implementation TodaysNoteWebViewController
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"todays_noteP",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
...
-(void) displayWebViewWithKey : (NSString *) key
{
    ...
    urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=todays_noteP&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
    NSLog(@"url=====>%@",urlString);
    } else {
        urlString = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=todays_noteP",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
    }
    [self.todaysNotesWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
//    UIImage *butImage = [[UIImage imageNamed:@"back-2"] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Todays Note" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.todaysNotesWebView canGoBack]) {
        [self.todaysNotesWebView goBack];
    }
    else
    {
