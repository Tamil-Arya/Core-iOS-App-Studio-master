...
@interface LocationWebViewController ()<APIProtocol>
{
    APIWithProtocol * api;
}
@end

@implementation LocationWebViewController
static int WEBSERVICE_TO_GET_Location_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"location_details",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Location_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
    ...
    NSString *urlString;
    if ([[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"parent"] || [[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"förälder"]) {
   urlString  = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=location_details&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"],key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
    NSLog(@"url=====>%@",urlString);
    } else {
         urlString  = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=location_details",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"],key];
    }
    [self.locationWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
-(void)Navigation_bar
{
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Current location" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.locationWebView canGoBack]) {
        [self.locationWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_Location_URL_TAG) {
        [self mStopIndicater];
