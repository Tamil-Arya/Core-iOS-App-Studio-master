//
//  Retrieval_Statistics_WebViewController.m
...
@interface Retrieval_Statistics_WebViewController ()
{
    APIWithProtocol * api;
}
...
@implementation Retrieval_Statistics_WebViewController
static NSInteger WEBSERVICE_TO_GET_Retrieval_Statistics_URL_TAG = 201;
- (void)viewDidLoad {
    [super viewDidLoad];
    [self Navigation_bar];
...
-(void) webserviceToGetWebViewURL  {
    ...
    NSDictionary * postDict = [[NSDictionary alloc] initWithObjectsAndKeys:@"retrieval_statistics",@"componenet_name",@"H67jdS7wwfh",@"securityKey",[[NSUserDefaults standardUserDefaults]valueForKey:@"user_id"],@"loginUserID",[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"],@"language",nil];
    ...
    NSLog(@"URL=>%@",url);
    [api CallWebserviceWithDataParametrs:postDict WithURL:url withMsgType:WEBSERVICE_TO_GET_Retrieval_Statistics_URL_TAG];
}
...
-(void) displayWebViewWithKey : (NSString *) key
{
    ...
    NSString *urlString;
    if ([[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"parent"] || [[[NSUserDefaults standardUserDefaults]valueForKey:@"user_type"]isEqualToString:@"förälder"]) {
   urlString  = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=retrieval_statistics&stud_id=%@",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key,[[NSUserDefaults standardUserDefaults]valueForKey:@"student_id"]];
    } else {
         urlString  = [NSString stringWithFormat:@"%@?authentication_token_SCM=%@&component_name=retrieval_statistics",[[NSUserDefaults standardUserDefaults]valueForKey:@"sub_domain"] ,key];
    }
    [self.retrieval_StatisticsWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:urlString]]];
...
-(void)Navigation_bar
{
 ...
 //   UIImage *butImage = [[UIImage imageNamed:@"back-2"] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
    ...
    self.navigationItem.title = [[NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:[[NSUserDefaults standardUserDefaults]valueForKey:@"langugae"] ofType:@"lproj"]] localizedStringForKey:@"Retrieval Statistics" value:@"" table:nil] ;
    
    
}

- (IBAction)backButtonClicked:(id)sender {
    if ([self.retrieval_StatisticsWebView canGoBack]) {
        [self.retrieval_StatisticsWebView goBack];
    }
    else
    {
...
{
    if (msgType == WEBSERVICE_TO_GET_Retrieval_Statistics_URL_TAG) {
        [self mStopIndicater];
